...
app.secret_key = os.environ.get("FLASK_SECRET_KEY", "sfce_secret_2025")
...
def download_receipt(receipt_no):
    try:
        ...
        <div class="footer"><p>This is a computer-generated receipt. No physical signature is required.</p><p>Smart Fee Collection Engine</p></div>
        </body></html>"""
...
@app.route('/admin/reminders/bulk_send', methods=['POST','OPTIONS'])
...
def chatbot():
    ...
    if not sid: 
        return jsonify({"reply":"Please provide your Student ID."})
    ...
    if not s: 
        conn.close()
        return jsonify({"reply":"Student ID not found."})
    ...
    if any(w in msg for w in ["pending","due","balance","remaining","unpaid"]):
        ...
        if last_pay: 
            return jsonify({"reply":f"Your last payment of ₹{last_pay['amount']:,.0f} was on {last_pay['payment_date']}."})
