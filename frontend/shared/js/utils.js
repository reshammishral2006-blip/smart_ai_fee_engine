...
async function sendCallReminder(phoneNumber) {
  const rawPhone = String(phoneNumber || '').trim();
  const digits = rawPhone.replace(/\D/g, '');
  const formattedPhone = rawPhone.startsWith('+91')
    ? '+91' + digits.slice(2)
    : '+91' + (digits.startsWith('91') ? digits.slice(2) : digits.replace(/^0/, ''));

  try {
    const response = await fetch(API + '/admin/send_call_reminder', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ to_phone: formattedPhone })
    });
    const data = await response.json();

    if (response.ok && data.status === 'sent') {
      alert('Call reminder sent! SID: ' + data.sid);
    } else {
      alert('Failed to send call: ' + (data.msg || 'Unknown error'));
    }
  } catch (error) {
    console.error(error);
    alert('Failed to send call reminder. Please check the server connection.');
  }
}
